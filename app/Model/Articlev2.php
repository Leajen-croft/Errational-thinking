...
class Article extends AppModel{
	...
	public $validate = array(
		...
		'filename_image' => array(
			...
			// http://book.cakephp.org/2.0/en/models/data-validation.html#Validation::mimeType
			'mimeType' => array(
				'rule' => array('mimeType', array('aplication/pdf')),
...
/*public function processUpload($check=array()) {
	...
	if (!empty($check['filename_image']['tmp_name'])) {
		...
		$filename_image = WWW_ROOT . $this->uploadDir . DS . Inflector::slug(pathinfo($check['filename_image']['name'], PATHINFO_FILENAME)).'.'.pathinfo($check['filename_image']['name'], PATHINFO_EXTENSION);
...
public function processUpload($check=array()) {
	...
	if (!empty($check['filename_magazine']['tmp_name'])) {
		...
		$filename_magazine = WWW_ROOT . $this->uploadDir . DS . Inflector::slug(pathinfo($check['filename_magazine']['name'], PATHINFO_FILENAME)).'.'.pathinfo($check['filename_magazine']['name'], PATHINFO_EXTENSION);
